...
elapsed = []
resid = []
for kmin in range(2,k):
    u = usave
    res = [hnorm(f - A * u)]
    start = timer()
    while(timer()-start<60):
        u = vcycle(k, kmin, A, u, f, 1)
...
for i in range(1,k-2):
    plt.semilogy(range(len(resid[i])),resid[i],label=i+2)
...
plt.figure(figsize=(8,6))
for i in range(1,k-2):
    plt.semilogy(np.linspace(0,elapsed[i],len(resid[i])),resid[i],label=i+2)
plt.xlabel("wall clock elapsed (s)")
plt.ylabel("residual")
plt.legend()
plt.show()

#Plot of residual after 10 iterations
plt.figure(figsize=(8,6))
plt.semilogy(range(2,10),[item[10] for item in resid])
plt.xlabel("kmin (kmax=10)")
plt.ylabel("residual at iteration 10")

#Plot of residual after 10 iterations
plt.figure(figsize=(8,6))
plt.plot(range(2,10),np.array([item[10] for item in resid])/np.array([item[9] for item in resid]))
plt.xlabel("kmin (kmax=10)")
plt.ylabel("convergence factor at iteration 10")