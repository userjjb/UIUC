...
def kappa(x, y):
    ...
    yi = (y + np.roll(y,-1))/2
    res = np.ones(3)
    res[(xi**2 + yi**2)**0.5 > 0.25] = 0.1
    return res[:, na, na]

def f(x, y):
    res = np.zeros(3)
    res[(x**2 + y**2)**0.5 <= 0.25] = 25
    return res
...
basis2E = np.array([basis2(0.5,0), basis2(0.5,0.5), basis2(0,0.5)])
...
exact = 1/(2*np.pi**2)
tt=exact*f(X[E[:,3:6]],Y[E[:,3:6]])-u[E[:,3:6]]
ttt=(detJm/6)*np.sum(tt**2,axis=1)
print(np.sqrt(np.sum(ttt)))
#print(-np.polyfit(np.log10([688,2752,11008]),np.log10([1.17281673507e-05,9.24139565044e-07,7.34392113784e-08]),1)[0])

perm = sparse.csgraph.reverse_cuthill_mckee(A)
def sp_permute(A, perm_r,perm_c):
    M,N=A.shape
    Pr = sparse.coo_matrix((np.ones(M), (perm_r, np.arange(N)))).tocsr()
    Pc = sparse.coo_matrix((np.ones(M), (np.arange(M),perm_c))).tocsr()
    return Pr.T*A*Pc.T

#plt.spy(sp_permute(A,perm,perm),markersize=2)